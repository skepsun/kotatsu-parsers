...
internal class LanraragiParser(context: MangaLoaderContext) :
    ...
    override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
        ...
        return pages.mapIndexedNotNull { index, any ->
            val s = any?.toString()?.trim().orEmpty()
            if (s.isEmpty()) return@mapIndexedNotNull null
            val full = if (s.startsWith("http", true)) s else "$base${if (s.startsWith("/")) s else "/$s"}"
            MangaPage(
