 ...
 */
@MangaSourceParser("NOVELIA", "轻小说机翻机器人", "zh", type = ContentType.NOVEL)
internal class Novelia(context: MangaLoaderContext) :
    ...
    private fun buildFilterTags(): Set<MangaTag> {
        ...
        // 来源站点过滤
        tags += MangaTag("来源: 全部", "provider:", source)
        tags += MangaTag("来源: Syosetu", "provider:syosetu", source)
        tags += MangaTag("来源: Kakuyomu", "provider:kakuyomu", source)
        tags += MangaTag("来源: Hameln", "provider:hameln", source)
        tags += MangaTag("来源: Pixiv", "provider:pixiv", source)
        tags += MangaTag("来源: Novelup", "provider:novelup", source)
        tags += MangaTag("来源: Alphapolis", "provider:alphapolis", source)
        
        // 类型过滤
        tags += MangaTag("类型: 全部", "type:0", source)
        tags += MangaTag("类型: 连载中", "type:1", source)
        tags += MangaTag("类型: 短篇", "type:2", source)
        tags += MangaTag("类型: 完结", "type:3", source)
        ...
        // 翻译状态
        tags += MangaTag("翻译: 全部", "translate:0", source)
        tags += MangaTag("翻译: 已翻译", "translate:1", source)
        tags += MangaTag("翻译: 未翻译", "translate:2", source)
