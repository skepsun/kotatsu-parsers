...
internal class Hanime1(
    ...
    // 站点标签（影片属性/人物关系/外貌身材/服饰道具/玩法调教/场景主题/性行为等）
    // 提取自 search.html 的 <input name="tags[]" value="..."> 列表
    private val SITE_TAG_VALUES: Set<String> = linkedSetOf(
        "無碼",
        "AI解碼",
        "中文字幕",
        "中文配音",
    ...
    override suspend fun getFilterOptions(): MangaListFilterOptions {
        val tags = buildStaticFilterTags()
        return MangaListFilterOptions(
            availableTags = tags,
        )
    }

    private fun buildStaticFilterTags(): Set<MangaTag> {
        val tags = LinkedHashSet<MangaTag>()

        // 基础类别（Genre）
        GENRE_VALUES.forEach { value ->
            tags.add(MangaTag(title = value, key = "genre:$value", source = source))
        }

        // 发布日期（简单选项）
        DATE_OPTION_VALUES.forEach { value ->
            tags.add(MangaTag(title = value, key = "date:$value", source = source))
        }

        // 发布年份（1990..2025）
        for (y in 2025 downTo 1990) {
            tags.add(MangaTag(title = "$y 年", key = "date-year:$y", source = source))
        }

        // 发布月份（1..12）
        for (m in 1..12) {
            tags.add(MangaTag(title = "$m 月", key = "date-month:$m", source = source))
        }

        // 时长
        DURATION_VALUES.forEach { value ->
            tags.add(MangaTag(title = value, key = "duration:$value", source = source))
        }

        // 站点标签（影片属性/人物关系等）
        SITE_TAG_VALUES.forEach { value ->
            tags.add(MangaTag(title = value, key = "tag:$value", source = source))
        }

        // 广泛配对（OR 组合标签）
        if (BROAD_SUPPORTED) {
            tags.add(MangaTag(title = "广泛配对", key = "broad:on", source = source))
        }
